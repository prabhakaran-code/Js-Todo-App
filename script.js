const API = "https://js-todo-app-1l6x.onrender.com/todos";
...
function updateTodoList() {
  ...
  if (currentFilter === "completed")
    filtered = allTodos.filter((t) => t.isCompleted);
...
async function finishEditing(newText, todoIndex) {
  if (!newText) return;

  if (
    allTodos.some(
      (t, i) =>
        t.text.toLowerCase() === newText.toLowerCase() && i !== todoIndex,
    )
  ) {
